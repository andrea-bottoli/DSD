...
public class DAOProvider {
	 ...
	 * @throws SQLException
	 */
	public static ResultSet SelectTableSecure(String table, String select,
			String where, String order, Connection con, Object[] parameters)
			throws SQLException {
		ResultSet resultSet = null;
		try {
			PreparedStatement command = con.prepareStatement(String.format(
					"select %s from %s %s %s", select, table, (where.trim()
							.equals("") ? "" : "where " + where), (order.trim()
							.equals("") ? "" : "order by " + order)));
			if (parameters != null) {
	 ...
	 * @throws SQLException
	 */
	public static int DeleteRowSecure(String table, String where,
			Connection con, Object[] parameters) throws SQLException {
		try {
			PreparedStatement command = con.prepareStatement(String.format(
					"delete from %s %s", table, (where.trim().equals("") ? ""
							: "where " + where)));
	 ...
	 * @throws SQLException
	 */
	public static int InsertRowSecure(String table, String fields,
			Connection con, Object[] valueArray) throws SQLException {
		try {
			String values = "";
			...
			command = con.prepareStatement(String.format(
	...
	public static int InsertRowsSecure(String table, String fields,
			Connection con, Object[][] valueArray) throws SQLException {
		try {
			String values = "(";
			...
			PreparedStatement command = con.prepareStatement(String.format(
					"insert into %s (%s) values %s", table, fields, rows));

			for (int i = 0; i < valueArray.length; i++) {
				for (int j = 0; j < valueArray[i].length; j++) {
					SetParameter(command, valueArray[i][j], i
							* valueArray[i].length + j + 1);
				}
			}
			command.executeUpdate();
		} catch (Exception ex) {
	 ...
	 * @throws SQLException
	 */
	public static int UpdateRowSecure(String table, String[] updateColumns,
			String where, Connection con, Object[] valueArray,
			Object[] wherePartParameters) throws SQLException {
		try {
			if (updateColumns.length != valueArray.length
			...
			for (int i = 0; i < wherePartParameters.length; i++) {
				SetParameter(command, wherePartParameters[i], valueArray.length
						+ i + 1);
			}
	...
	public static int UpdateRowsSecure(String table, String[] fields,
			Connection con, Object[][] valueArray) throws SQLException {
		try {
			String values = "(";
			if (valueArray[0].length > 0) {
			...
			PreparedStatement command = con.prepareStatement(String.format(
					...
					StringUtils.join(fields, ','), rows, onDuplicateKey));
	 ...
	 * @throws SQLException
	 */
	private static void SetParameter(PreparedStatement command, Object object,
