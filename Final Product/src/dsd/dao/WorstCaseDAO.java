...
public class WorstCaseDAO {
	...
	private static String[] fields = new String[] { "pylon_number", "N", "Tx",
			"Ty", "Mx", "My", "M", "cs", "comb_number", "timestamp" };

	public static int InsertWorstCaseData(List<WorstPylonCase> listOfData,
		...
		try {
			...
			try {
				...
				counter += DAOProvider.InsertRowsSecure(tableName,
						StringUtils.join(fields, ','), con,
	...
	public static int UpdateWorstCaseData(List<WorstPylonCase> listOfData,
			boolean traffic, boolean debris) {
		try {
			Connection con = DAOProvider.getDataSource().getConnection();
			try {
				...
				DAOProvider.UpdateRowsSecure(tableName,
						PrepareColumnsForUpdate(), con,
	...
	public static ArrayList<WorstPylonCase> GetAllForPeriod(Calendar startDate,
			Calendar endDate, boolean traffic, boolean debris) {
		try {
			Connection con = DAOProvider.getDataSource().getConnection();
			...
			try {
				...
				ResultSet results = DAOProvider.SelectTableSecure(tableName,
						"*", " timestamp >= ? and timestamp <= ? ", "", con,
						parameters);
				while (results.next()) {
					WorstPylonCase dataTuple = new WorstPylonCase(
							results.getInt(fields[0]));
					...
					dataTuple.setTimestamp(results.getTimestamp(fields[9])
							.getTime());
	...
	private static Object[][] PrepareMultipleValuesForInsert(
			List<WorstPylonCase> listOfData) {
	...
	private static Object[][] PrepareMultipleValuesForUpdate(
			List<WorstPylonCase> listOfData) {
	...
	private static String GetTableNameForDataType(boolean traffic,
			boolean debris) {
		if (!traffic && !debris) {
			return tableName00;
